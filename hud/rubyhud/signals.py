...
class Snapshot:
    ...
    reverse: bool = False
    # ---- body / safety indicators (0x43E doors+trunk, 0x477 BSM) ----------
...
MX5_ID_DOORS = 0x43E  # HS_BCMM (dec 1086): DoorLeft@36, DoorRight@37, Trunk@47 (1-bit)
MX5_ID_BSM  = 0x477  # HS_IC (dec 1143): BSM_Right@1, BSM_Warning@2, BSM_Left@15 (1-bit)
...
class DataLayer:
    ...
    def _decode_mx5(self, arb, data: bytes) -> None:
        ...
        # mapping confirmed on the car by opening each door (see can/README).
            ...
            if tk is not None:
                self._trunk = bool(tk)
        # ---- 0x477 HS_IC: blind-spot monitor (1-bit each, Motorola @0+) ------
